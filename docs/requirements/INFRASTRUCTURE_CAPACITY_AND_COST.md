...
但不足以一次性采购“生产终态”。云申请不再作为当前开发前置，推荐按证据分四步投入：

1. 先用现有本地资源完成发布候选、故障恢复和部署验证器，新增云成本 **$0**；
2. 本地 L3 通过后再执行已批准的 14 天 benchmark，预算上限 **$150 / ¥1,080**；
3. 数据契约和 24h 连续运行通过后，批准 3 个月研究环境，预算上限
   **$2,600 / ¥18,700**；
4. 只有在 30 天容量曲线稳定后，再选择 12 个月目标子集方案，按需价预算
...
- S0 申请前：`make verify-release` 在 clean commit 上通过，部署后验证脚本和阈值已冻结；
- S0 创建前：云账号 owner、预算告警、数据源清单、采样范围和销毁规则明确；
